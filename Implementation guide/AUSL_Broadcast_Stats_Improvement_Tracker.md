...
Target outcome for the current phase: turn official notes into a small,
exact-game, balanced, source-labeled rundown and make producer prep
nonduplicative.

Current implementation unit: none. The independent Phase 4 stability-audit
remediation passed its exact-game live, historical-prep, lineup round-trip,
starter-position, packet-label, media-approval, Tk ownership, refresh rollback,
and portable-build acceptance gates.

The master list below is the single source of task status. No Phase 4 item is
complete until its tests and stated acceptance behavior pass.

Status: **COMPLETE — PHASE 4 STABILITY REMEDIATION — 2026-07-22**
...
- Refresh failure/staleness checks:
- Rehearsal result:
