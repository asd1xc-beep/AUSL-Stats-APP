...
PENDING = "CONNECTION ENGINE COMPLETE — CONNECTION REVIEW PENDING"
...
def test_review_packet_records_all_batch_approvals_and_connection_gate():
    ...
    assert "must not enter producer-facing use" in text
...
def test_interim_acceptance_record_is_honest_about_phase_boundary():
    ...
    assert "Connection wording review and approval: pending" in text
    assert "8 candidates" in text
...
def test_connection_specification_documents_fail_closed_relationship_rules():
    ...
    for required in (
        ...
        "COLLEGE-007 remains incomplete",
    ):
        assert required in text
